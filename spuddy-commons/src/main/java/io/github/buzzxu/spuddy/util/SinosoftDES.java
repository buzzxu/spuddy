...
public class SinosoftDES {

	private static final byte[] IP_Table = { 58, 50, 42, 34, 26, 18, 10, 2, 60,
	...
	private static final byte[] P_Table = { 16, 7, 20, 21, 29, 12, 28, 17, 1,
			...
			6, 22, 11, 4, 25 };
	private static final byte[] PC1_Table = { 57, 49, 41, 33, 25, 17, 9, 1, 58,
	...
	private static final byte[] PC2_Table = { 14, 17, 11, 24, 1, 5, 3, 28, 15,
	...
	private static final byte ENCRYPT = 1;
	private static final byte DECRYPT = 2;
	private byte[] key;
	private boolean is3DES;
	private final byte[][][] SubKey;
	private final byte[] Tmp;
	private final byte[] deskey;
	private final byte[] MR;
	...
	private void sFunc(byte[] abyte0, byte[] abyte1) {
		for (byte byte0 = 0; byte0 < 8; byte0++) {
			...
			byte byte2 = (byte) ((abyte1[byte0 * 6 + 1] << 3)
					+ (abyte1[byte0 * 6 + 2] << 2)
					+ (abyte1[byte0 * 6 + 3] << 1) + abyte1[byte0 * 6 + 4]);
	...
	private byte[] doEncrypt(byte[] abyte0) throws Exception {
		byte[] abyte1 = usingDES(abyte0, key, (byte) 1);
		return abyte1;
	}

	private byte[] doDecrypt(byte[] abyte0) throws Exception {
	...
	private byte[] hexStr2ByteArr(String s) throws Exception {
	...
	public String hex(String text, String secret) throws Exception {
		key = secret.getBytes();
		byte[] abyte0 = doEncrypt(text.getBytes());
		String s3 = toHexString(abyte0);
		return s3.trim();
	}
	public String dec(String encryption,String secret) throws Exception {
		key = secret.getBytes();
		byte[] abyte1 = hexStr2ByteArr(encryption);
	...
	public static void main(String[] args) throws Exception {
		SinosoftDES s = new SinosoftDES();
		System.out.println(s.hex("dfd徐翔","xux"));
		//E43279CCABA9D448AD0E52845F46FC04
		System.out.println(s.dec("E43279CCABA9D448AD0E52845F46FC04","xux"));
	}
}
